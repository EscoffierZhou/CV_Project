"""
零样本目标跟踪 Gradio 界面 (TensorRT 加速版)
"""
import gradio as gr
import cv2
import numpy as np
...
try:
    ...
    print("🚀 TensorRT detected! Using OSTrackFerrariTRT (Ferrari Turbo Mode)")
    TRACKER_CLASS = OSTrackFerrariTRT
    TRACKER_NAME = "OSTrack Ferrari (TensorRT Turbo)"
...
def process_video(video_path, x1, y1, x2, y2):
    ...
    output_path = OUTPUT_DIR / f"{video_stem}_trt_{timestamp}.mp4"
    
    # VideoWriter will be initialized after tracker setup
    ...
    print(f"🎯 [TRT] 开始跟踪，初始边界框: {init_bbox}")
    ...
    while True:
        ...
        # 帧计数器（始终显示）
        cv2.putText(vis_frame, f"Frame: {frame_idx}/{total_frames} (TRT)", (10, 30),
    ...
    # 保存统计数据到文件
    video_stem = Path(video_path).stem
    stats_file_path = OUTPUT_DIR / f"{video_stem}_trt.txt"
    try:
...
# Gradio 界面
with gr.Blocks(title="🎯 零样本目标跟踪器 (TensorRT)") as demo:
    gr.Markdown(f"""
    # 🏎️ {TRACKER_NAME}
    ...
    **使用说明:**
    ...
    with gr.Row():
        with gr.Column():
            ...
            track_btn = gr.Button("🚀 开始跟踪 (TensorRT)", variant="primary", size="lg")
